<template>
  <div class="administrator-drawers-box">
    <Drawer :mask-closable="false" :title="drawerTitle" width="640" v-model="drawer1" @on-close="hideDrawer1" class="drawer-box common-style">
      <Card class="card">
        <Icon type="ios-information-circle" class="icon" />
        <p class="card-p">快速排班将按照以下规则执行</p>
        <p class="card-p">1.人员范围：选定指定员工</p>
        <p class="card-p">2.班次规则：以管理人员 - 列表中人员所选班次进行排班</p>
        <p class="card-p">3.工作日规则：除特定设置工作日班次（如：家装）人员，排班将结合双休&假期日历设置，仅针对工作日进行排班</p>
      </Card>
      <h3 class="title-h">排班信息</h3>
      <Form :label-width="80">
        <FormItem label="排班期间">
          <DatePicker :value="datePicker" type="daterange" format="yyyy-MM-dd"  :editable="false" placeholder="请选择" style="width: 200px" @on-change="getPickerDate"></DatePicker>
        </FormItem>
        <FormItem label="备注">
      ...
      >
        <template slot-scope="{ row, index}" slot="count">
          <Button type="text" style="margin-right: 10px" @click="showDrawer2(row.id)">{{row.count}}</Button>
        </template>

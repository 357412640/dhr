...
export const noData = `<div style="margin: 20px">
                    <img src="${noDataDefault}" alt="" width="101" height="72">
                    <span>暂无数据</span>
                </div>`
export const noDataLong = `<div style='position:absolute;left:45%;bottom:30px;'>暂无数据</div>`
...
const showThisMenuEle = (item, menu) => {
  if (item.meta && item.meta.menuId) {
    if (~menu.indexOf(item.meta.menuId)) return true
    else return false
  } else return true
}
/**
 ...
 * @returns {Array}
 */
export const getMenuByRouter = (list, menu) => {
  let res = []
  forEach(list, item => {
    if (!item.meta || (item.meta && !item.meta.hideInMenu)) {
      ...
      // debugger
      if ((hasChild(item) || (item.meta && item.meta.showAlways)) && showThisMenuEle(item, menu)) {
        obj.children = getMenuByRouter(item.children, menu)
        if (obj.children.length) {
          res.push(obj)
        }
      } else {
        if (item.meta && item.meta.href) obj.href = item.meta.href
        if (showThisMenuEle(item, menu)) res.push(obj)
      }
    }
  })
  return res
...
const hasAccess = (menu, route) => {
  if (route.meta && route.meta.menuId) return ~menu.indexOf(route.meta.menuId)
  else return true
}
...
// Date返回年月日组成的字符串 2019-01-16 12:12:12
export const resetTime5 = (t) => {
  let date = new Date(t)
  let year = date.getFullYear()
  let month = (date.getMonth() + 1).toString().length > 1 ? date.getMonth() + 1 : '0' + (date.getMonth() + 1)
  let day = date.getDate().toString().length > 1 ? date.getDate() : '0' + date.getDate()
...
export const getMonthList = (str) => {
  ...
  let nexYear = curYear + 1
  for (let i = 1; i < curMonth; i++) {
    let month = i.toString()
    nexArr.push(`${nexYear}${str}${month.length < 2 ? '0' + month : month}`)
    curArr.push(`${curYear}${str}${month.length < 2 ? '0' + month : month}`)
  }
  for (let i = curMonth; i <= 12; i++) {
    let month = i.toString()
    preArr.push(`${preYear}${str}${month.length < 2 ? '0' + month : month}`)
    curArr.push(`${curYear}${str}${month.length < 2 ? '0' + month : month}`)
  }
  return [...preArr, ...curArr, ...nexArr]
}
